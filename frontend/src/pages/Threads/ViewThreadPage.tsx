...
export function ViewThreadPage() {
    ...
    const { thread, addLink } = useThread(id ?? null);
    ...
    return (
        <main className="min-h-[calc(100dvh-80px)] flex flex-col items-center bg-gradient-to-br from-gray-100 via-gray-50 to-gray-200 text-gray-900 dark:from-gray-900 dark:via-gray-800 dark:to-gray-950 dark:text-white transition-colors">
            <section className="container mx-auto px-4 py-10 space-y-6">
                {thread && (
                    <header className="rounded-2xl px-6 py-5 shadow-lg border bg-white/90 border-gray-200 dark:bg-gray-900/70 dark:border-gray-800 transition-colors">
                        ...
                        {Array.isArray(thread.tags) && thread.tags.length > 0 && (
                            <ul className="flex gap-2 flex-wrap mt-2">
                                {thread.tags.map((tg, i) => (
                                    <li key={`${tg}-${i}`} className="text-xs bg-indigo-100/60 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 px-1.5 py-0.5 rounded-xl">
                                        {tg}
                                    </li>
                                ))}
                            </ul>
                        ...
                        <div className="flex items-center gap-2 mt-4">
                            <div className="flex items-center gap-2 rounded-xl px-3 py-1 bg-gray-100 dark:bg-gray-800/70">
                                <select
                ...
                <section className="space-y-4">
                    {mode === 'timeline' && (
                        <ThreadTimeline items={items.filter(i => i.links)} />
                    )}
                    {mode === 'stack' && (
                        <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {items.filter(i => i.links).map(item => (
                                <li key={item.id} className="rounded-xl border bg-white/90 border-gray-200 dark:bg-gray-900/70 dark:border-gray-800">
                                    {item.links && <LinkCard link={item.links} />}
                                </li>
                            ))}
                        </ul>
                    )}
